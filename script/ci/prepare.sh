...
if [ ! -e $ERLANG_PATH/bin/erl ]; then
  ...
  cd $ERLANG_PATH
  ./configure --enable-smp-support --enable-m64-build --disable-native-libs \
              --disable-sctp --enable-threads --enable-kernel-poll --disable-hipe \
              --without-javac
  make
...
# Fetch and compile dependencies and application code.
export MIX_ENV="test"
